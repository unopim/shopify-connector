        ...
        foreach($importerConfig as $name => $config) {
            ...
            foreach ($config['filters']['fields'] as $key => $filter) {
                ...
                if ($filter['type'] == 'select' || $filter['type'] == 'multiselect') {
                    if (! empty($filter['dependent']) && is_array($filter['dependent'])) {
                        $queryParams = [];

                        foreach ($filter['dependent'] as $dependentField) {
                            if (isset($values[$dependentField]) && $values[$dependentField] !== '') {
                                $queryParams[$dependentField] = $values[$dependentField];
                            }
                        }

                        if (! empty($queryParams)) {
                            $importerConfig[$name]['filters']['fields'][$key]['query_params'] = $queryParams;
                        }
                    }
        ...
        app.component('v-import-filter-fields', {
            ...
            methods: {
                ...
                emitChangeEvent(fieldName, value) {
                    const dependentFields = this.fields.filter((item) =>
                        Array.isArray(item.dependent) && item.dependent.includes(fieldName)
                    );

                    dependentFields.forEach((item) => {
                        if ('undefined' === typeof this.$refs[item.name]) {
                            return;
                        }

                        const dependentRef = Array.isArray(this.$refs[item.name])
                            ? this.$refs[item.name][0]
                            : this.$refs[item.name];

                        const previousValue = dependentRef.params?.[fieldName] ?? item.query_params?.[fieldName] ?? '';
                        ...
                        dependentRef.selectedValue = item.type === 'multiselect' ? [] : null;
                        dependentRef.optionsList = [];
                    });
